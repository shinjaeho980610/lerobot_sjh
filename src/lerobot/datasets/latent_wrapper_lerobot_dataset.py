...
def q_cur_n(sys1_hz : float, sys2_hz : float, n : int) -> int:
    return math.floor((n * sys2_hz) / sys1_hz)

def q_use_n(q_cur : int) -> int:
    return max(q_cur-1, 0)

def n_obs_n(q_use : int, sys1_hz : float, sys2_hz : float) -> int:
    return math.ceil((q_use * sys1_hz) / sys2_hz)

def hz_parser(n : int, sys1_hz : float = 15, sys2_hz : float = 5) -> int:
    q_cur = q_cur_n(sys1_hz, sys2_hz, n)
    q_use = q_use_n(q_cur)
    n_obs = n_obs_n(q_use, sys1_hz, sys2_hz)
    return n_obs
...
class LatentWrapperDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.latent_root = f'{base_dataset.root}/{latent_root}'
        ...
        self.use_hz_parser = use_hz_parser
        self.sys1_hz = sys1_hz
        self.sys2_hz = sys2_hz
    ...
    def __getitem__(self, idx):
        ...
        if self.use_hz_parser:
            selected_idx = hz_parser(
                n = frame_idx,
                sys1_hz = self.sys1_hz,
                sys2_hz = self.sys2_hz
            )
        else:
