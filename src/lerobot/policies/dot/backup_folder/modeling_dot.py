    ...
    def __init__(self, config: DOTConfig):
        ...
        if len(self.image_names) > 0:
            ...
            if self.config.vision_backbone in ["resnet18convfilm", "resnet34convfilm", "resnet50convfilm"]:
                ...
                self.vision_proj = nn.Linear(self.backbone._output_channels, self.config.dim_model)
                #self.vision_backbone = add_lora_to_backbone(self.vision_backbone, rank=config.lora_rank)
    ...
    def _process_inputs(self, batch):
        ...
        for state in self.projections_names:
            ...
            if batch_state in batch:
                ...
                bs, n_items, *obs_shape = batch[batch_state].shape
                if state == "images" and hasattr(self, "backbone"):
                    # (bs, n_items, C, H, W) -> (bs*n_items, C, H, W)
                    x = batch[batch_state].view(bs * n_items, *obs_shape)
                    # language는 사용 안 함; 오직 latent만 사용
                    print(batch['latent'].shape)
                    #latent = self._get_latent_for_film(batch, bs, n_items)  # (bs*n_items, D)
                    feat = self.backbone(x, latent)                         # (bs*n_items, C', H', W')  ← FiLM은 (x, lang_emb) 시그니처
                    pooled = self.pool(feat).flatten(1)                 # (bs*n_items, C')
                    enc = self.vision_proj(pooled).view(bs, n_items, -1)
                else:
                    enc = self.projections[state](batch[batch_state].view(bs * n_items, *obs_shape)).view(
                        bs, n_items, -1
                    )
...
class DOTPolicy(PreTrainedPolicy):
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor]) -> Tensor:
        return torch.randn((batch['state'].shape))
...
def add_lora_to_backbone(backbone, rank=4, verbose=True):
    ...
    for name, param in backbone.named_parameters():
        if ("lora_" in name) or name.startswith("fc") or ("lang_proj" in name) or ("_film_layers" in name):
