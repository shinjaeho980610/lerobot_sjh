...
class FiLMLayer(ConvBase):
    ...
    def __init__(
        ...
        #print("filmlayer channels", channels)
        # added codes by hbo on 241008 to add non-linearity
        if lang_emb_dim > channels * 2:
            # 입력 차원을 절반씩 나누어 128까지 감소시키는 레이어들을 생성합니다.
            layers = []
            dim = lang_emb_dim
            while dim > channels * 2:
                next_dim = max(dim // 2, channels * 2)
                layers.append(nn.Linear(dim, next_dim))
                layers.append(nn.ReLU())
                dim = next_dim
            self.lang_proj = nn.Sequential(*layers)
        else:
            # 입력 차원이 128 이하인 경우 단일 선형 레이어를 사용합니다.
...
class ResNet18ConvFiLM(ConvBase):
    ...
    def __init__(
        ...
        lang_emb_dim=768,  # modified by hbo on 240925 for openvla embedding
        input_channel=3,
        pretrained=False,
        input_coord_conv=False,
    ):
