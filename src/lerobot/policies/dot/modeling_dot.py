...
from lerobot.policies.act.base_nets import ResNet18ConvFiLM, ResNet34ConvFiLM, ResNet50ConvFiLM
...
class DOT(nn.Module):
    def __init__(self, config: DOTConfig):
        ...
        if len(self.image_names) > 0:
            ...
            if self.config.vision_backbone in ["resnet18convfilm", "resnet34convfilm", "resnet50convfilm"]:
                # 251007 cam별 encoder추가
                film_classes = {
                        "resnet18convfilm": ResNet18ConvFiLM,
                        "resnet34convfilm": ResNet34ConvFiLM,
                        "resnet50convfilm": ResNet50ConvFiLM,
                    }
                if self.config.use_separate_rgb_encoder_per_camera:
                    backbone_lst = [film_classes[self.config.vision_backbone](
                        pretrained=True,
                        lang_emb_dim=self.config.input_features["latent"].shape[0]
                    ) for _ in range(len(self.image_names))]

                    self.backbone = nn.ModuleList(backbone_lst)

                    out_C, out_H, out_W = self.backbone[0].output_shape(
                        (3, self.config.rescale_shape[0], self.config.rescale_shape[1])
                        #self.config.input_features["observation.left_image"].shape
                    )
                else:
                    self.backbone = film_classes[self.config.vision_backbone](
                        pretrained=True,
                        lang_emb_dim=self.config.input_features["latent"].shape[0]
                    )

                    # (C, H, W) 형태의 피처맵 크기 추론
                    out_C, out_H, out_W = self.backbone.output_shape(
                        (3, self.config.rescale_shape[0], self.config.rescale_shape[1])
                        #self.config.input_features["observation.left_image"].shape
                    )
                # 전역 평균 풀링으로 (C, 1, 1) 만들고, C를 Linear 입력으로 사용
                self.backbone_avgpool = nn.AdaptiveAvgPool2d((1, 1))  # ✅ (H, W) 지정
                self.backbone_fc = nn.Linear(out_C, self.config.dim_model)  # ✅ 채널 수를 in_feat
                # modified by sjh on 250922 [1]
                # vision backbone을 lora를 이용하여 학습시킴
                #self.projections["images"] = add_lora_to_backbone(self.backbone, rank=config.lora_rank)
                self.projections["images"] = None
            else:
        ...
        self.projections['latent'] = None

        # modified by sjh on 250922 [2]
        # env_feature 주석처리
        ...
        obs_mapping = {
            ...
            # modified by sjh on 250922 [3]
            # obs_mapping에 latent추가 >> 추론중 정규화를 위해서
            "latent": "latent",
    ...
    def _process_inputs(self, batch):
        ...
        # modified by sjh on 250922 [4]
        # image와 latent는 하나의 projection을 사용하기 때문에, latent용 projection이 생성되지 않도록 해야됨
        #for state in self.projections_names:
        for state in ['state', 'images']:
            batch_state = self.obs_mapping[state]
            if batch_state in batch:
                # 251007 n_obs에서 n_items로 수정 >> batch에서 나오는 건 
                # [left_1, right_1, mid_1, left_2, right_2, mid_2, left_3, right_3, mid_3]
                # 이므로 n_obs >> n_items
                #bs, n_obs, *obs_shape = batch[batch_state].shape
                bs, n_items, *items_shape = batch[batch_state].shape
                #print(bs, n_obs, *obs_shape)
                #print(batch['latent'].shape)
                if state == 'images' and "convfilm" in self.config.vision_backbone:
                    # 251007 cam별 encoder추가
                    if self.config.use_separate_rgb_encoder_per_camera:
                        n_cam = len(self.image_names)
                        n_obs = n_items // n_cam

                        enc = torch.empty(bs, n_items, self.config.dim_model, device = batch[batch_state].device, dtype=batch[batch_state].dtype)
                        for i in range(n_cam):
                            x_i = batch[batch_state][:, i::n_cam].contiguous().view(bs*n_obs, *items_shape)
                            z_i = batch["latent"][:,  i::n_cam].contiguous().view(bs*n_obs, -1)
                            feat = self.backbone[i](x_i,z_i)
                            pooled = self.backbone_avgpool(feat).flatten(1)
                            tok_i = self.backbone_fc(pooled).view(bs, n_obs, -1)
                            enc[:, i::n_cam, :] = tok_i
                    else:
                        # (bs, n_items, C, H, W) -> (bs*n_items, C, H, W)
                        x = batch[batch_state].view(bs * n_items, *items_shape)
                        feat = self.backbone(x, batch["latent"].view(bs * n_items, -1))
                        pooled = self.backbone_avgpool(feat).flatten(1)
                        enc = self.backbone_fc(pooled).view(bs, n_items, -1)
                else:
                    enc = self.projections[state](batch[batch_state].view(bs * n_items, *items_shape)).view(
                        bs, n_items, -1
                    )
...
class DOTPolicy(PreTrainedPolicy):
    ...
    def _prepare_batch_for_inference(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        if "observation.images" in batch:
            ...
            # modified by sjh on 250922 [6]
            batch["latent"] = torch.stack([batch["latent"] for k in range(len(self.image_names))], dim=2).flatten(
                1, 2
            ).squeeze(2)
        return batch
    ...
    def predict_action_chunk(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        #actions = self.unnormalize_outputs({"action": actions_pred})["action"]
        #return actions
    ...
    def forward(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        # modified by sjh on 250922 [7]
        # obs_mapping에 latent가 추가됨!
        #if "convfilm" in self.config.vision_backbone:
        #    # added by hbo on 250913
        #    batch['latent'] = torch.cat(
        #        [
        #            batch['latent'][:, lookback_ind: lookback_ind + 1],
        #            batch['latent'][:, 2 * self.config.lookback_aug + 1:],
        #        ],
        #        1,
        #        )
        ...
        if self.state_noise is not None:
            for k in self.model.obs_mapping.values():
                # modified by sjh on 250922 [8]
                # latent에 noise가 들어가지 않도록 수정 ...
                #if k != "observation.images":
                if k != "observation.images" and k != 'latent':
