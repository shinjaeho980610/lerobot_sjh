...
class DualProcess_VLA:
    def __init__(self, system1_cfg_path, system2_model_path, system1_hz=25, system2_hz=1, mode="simulation"):
        ...
        self.system2_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            system2_model_path, torch_dtype="auto", device_map="auto"
        )
        self.processor = AutoProcessor.from_pretrained(system2_model_path)
    ...
    def system2_thread(self, obs_dict, goal_dict):
        while self.thread_running:
            latent = self.extract_latent(obs_dict, goal_dict)
            if not self.latent_queue.empty():
                _ = self.latent_queue.get()
            self.latent_queue.put(latent)

    def extract_latent(self, obs_dict, goal_dict):
        ...
        query = (
            ...
            'Picture 2: <|vision_start|><|image_pad|><|vision_end|> - Captured by the top-right camera, showing both the robot and its environment. '
            'Picture 3: <|vision_start|><|image_pad|><|vision_end|> - Captured by the top-left camera, showing both the robot and its environment.<|im_end|>\n'
            '<|im_start|>user\n'
            f'How should you move when you need to {goal_dict["lang"].lower()}?'
            '<|im_start|>assistant\n'
        )
    ...
    def start_system2_thread(self, obs_dict, goal_dict):
        if self.mode == "thread" and not self.thread_running:
            self.thread_running = True
            thread = threading.Thread(target=self.system2_thread, args=(obs_dict, goal_dict))
            thread.daemon = True
            thread.start()
    ...
    def forward_system1(self, obs_dict, goal_dict):
        #print(self.current_step, self.step_interval)
        if self.mode == "simulation":
            if self.current_step % self.step_interval == 0:
                self.current_latent = self.extract_latent(obs_dict, goal_dict)
        ...
        img_e = obs_dict['robot0_eye_in_hand_image']
        img_r = obs_dict['robot0_agentview_right_image']
        img_l = obs_dict['robot0_agentview_left_image']
